...
$program = array_map('intval', explode(',', trim(file_get_contents(__DIR__.'/day11.txt'))));
$intputer = new Intputer($program);
...
do {
    [ $x, $y ] = $robotPosition;
    $panelColour = (empty($grid[$x][$y])) ? 0 : 1; // 0 = black, 1 = white

    $newPanelColour = $intputer->run([ $panelColour ]);
    $turn = $intputer->run();

    $grid[$x][$y] = $newPanelColour;

    switch ($robotDirection) {
        case '^':
            $robotDirection = ($turn === 0) ? '<' : '>';
            $robotPosition = ($turn === 0) ? [ $x - 1, $y ] : [ $x + 1, $y ];
            break;
        case '>':
            $robotDirection = ($turn === 0) ? '^' : 'v';
            $robotPosition = ($turn === 0) ? [ $x, $y - 1 ] : [ $x, $y + 1 ];
            break;
        case 'v':
            $robotDirection = ($turn === 0) ? '>' : '<';
            $robotPosition = ($turn === 0) ? [ $x + 1, $y ] : [ $x - 1, $y ];
...
$yVals = [];
foreach ($grid as $col) {
    $yVals = array_merge($yVals, array_keys($col));
}

$minX = min(array_keys($grid));
$maxX = max(array_keys($grid));
$minY = min($yVals);
$maxY = max($yVals);
